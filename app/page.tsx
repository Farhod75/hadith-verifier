...
const DUA_EXAMPLES = {
  wrong: `اللهم أغنني بفضلك عمّن سواك و بحَلَالَك عن حَرَامِك\nАллагьумма агъними би фадлика 'амман сивак ва би хIалалика 'ан хIарамик`,
  ...
  async function analyze() {
    ...
    try {
      ...
      if (image) {
        ...
        fd.append('image', image); fd.append('lang', replyLang)
        ...
        res = await fetch('/api/analyze', {
          ...
          body: JSON.stringify({ postText, lang: replyLang })
        })
      }
      const data = await res.json()
      ...
      setStats(p => ({
        total:      p.total + 1,
        fabricated: p.fabricated + (['fabricated','weak'].includes(data.verdict) ? 1 : 0),
        authentic:  p.authentic  + (data.verdict === 'authentic' ? 1 : 0)
      }))
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="flex gap-2 mt-3 items-center flex-wrap">
                <button onClick={analyze} disabled={loading || (!postText.trim() && !image)}
                  ...
                  {loading
                    ? <><span className="w-3 h-3 border-2 border-white/40 border-t-white rounded-full animate-spin" />{image ? tr.readingImage : tr.analyzing}</>
                    : image ? tr.analyzeScreenshot : tr.analyzePost}
                </button>
                ...
                <div className={`rounded-xl border p-4 ${vs.bg} ${vs.border}`}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-gray-500">
                      {result.confidence === 'high' ? tr.highConfidence : result.confidence === 'medium' ? tr.mediumConfidence : tr.lowConfidence}
                    </span>
                  </div>
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      {['fabricated','weak'].includes(result.verdict) && (
                        <button onClick={() => setTab('admin')}
                          className="text-sm px-4 py-1.5 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50">{tr.viewInAdminQueue}</button>
                      )}
                    </div>
                  </div>
                )}
                        ...
                        <div className="mb-3">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {item.red_flags.slice(0, 3).map((f, i) => (
                              <span key={i} className="text-xs bg-red-50 text-red-700 px-2 py-0.5 rounded-full border border-red-100">{f}</span>
                            ))}
                          </div>
                        </div>
                      )}
