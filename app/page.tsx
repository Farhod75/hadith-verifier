...
const TIER_CONFIG = {
  tier1: { label: 'Tier 1', dot: 'bg-emerald-500', badge: 'bg-emerald-50 text-emerald-800' },
  tier2: { label: 'Tier 2', dot: 'bg-blue-500', badge: 'bg-blue-50 text-blue-800' },
  tier3: { label: 'Tier 3', dot: 'bg-amber-500', badge: 'bg-amber-50 text-amber-800' },
}
...
export default function Home() {
  ...
  async function analyze() {
    ...
    } catch { alert('Analysis failed. Check your API key.') }
    setLoading(false)
  }
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="flex gap-2 mt-3 items-center flex-wrap">
                <button onClick={analyze} disabled={loading || !postText.trim()}
                  ...
                  {loading ? <><span className="w-3 h-3 border-2 border-white/40 border-t-white rounded-full animate-spin"/> Analyzing...</> : 'Analyze post'}
                </button>
                <button onClick={() => { setPostText(''); setResult(null) }}
                  className="text-sm px-4 py-2 rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50">
                  Clear
                </button>
                <div className="ml-auto flex gap-1">
                  <span className="text-xs text-gray-400 self-center mr-1">Reply in:</span>
                          ...
                          return (
                            <div key={i} className="flex gap-2 py-2 border-b border-gray-100 last:border-0">
                              ...
                              <div>
                                ...
                                <span className={`text-xs ml-2 px-2 py-0.5 rounded-full ${tc.badge}`}>{tc.label}</span>
        ...
        {tab === 'sources' && (
          <div className="space-y-4">
            {[
              {
                tier: 'Tier 1 — Primary collections (highest authority)',
                color: 'emerald',
                sources: [
                  { name: 'Dorar.net (الدرر السنية)', desc: '520,000+ hadiths, 400 volumes, 150,000 narrator biographies. JSON API. Arabic-primary.', url: 'https://dorar.net' },
              ...
              {
                tier: 'Tier 2 — Scholarly fatwa bodies',
                color: 'blue',
                sources: [
                  ...
                  { name: 'IslamWeb.net', desc: 'Full takhrij: sanad, matn, narrator biography, hadith ranking. Best for chain verification.', url: 'https://islamweb.net' },
                  { name: 'Yaqeen Institute', desc: 'Peer-reviewed Islamic scholarship in English. Hadith criticism methodology.', url: 'https://yaqeeninstitute.org' },
                  { name: 'Islamhouse.com', desc: 'Uzbek, Russian, Arabic, 100+ languages. Books on fabricated hadiths. Good for Central Asia.', url: 'https://islamhouse.com' },
                ]
              },
              {
                ...
                color: 'amber',
                sources: [
                  { name: 'HadithAPI.com', desc: 'Free REST API. Bukhari, Muslim, Abu Dawud, Tirmidhi in multiple languages.', url: 'https://hadithapi.com' },
                  { name: 'AboutIslam.net', desc: 'Scholar Q&A on hadith authenticity. Good for common viral fabrications.', url: 'https://aboutislam.net' },
                  { name: 'AlSunnah.com', desc: 'Full Kutub al-Sittah searchable. Good cross-reference for any collection.', url: 'https://alsunnah.com' },
                ]
              }
            ].map(section => (
        ...
        {tab === 'admin' && (
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              {queueLoading && <div className="text-sm text-gray-400 py-4 text-center">Loading queue...</div>}

              {!queueLoading && queue.length === 0 && (
                <div className="text-sm text-gray-400 py-8 text-center">
                  No flagged posts yet. Analyze posts to populate the queue.
                </div>
              )}
              ...
              {queue.map(item => (
                <div key={item.id} className="flex gap-3 py-3 border-b border-gray-100 last:border-0">
                  ...
                  <div className="flex flex-col gap-1 flex-shrink-0">
                    <button onClick={() => { navigator.clipboard.writeText(item.suggested_comment); alert('Comment copied!') }}
                      className="text-xs px-3 py-1 rounded border border-green-200 bg-green-50 text-green-700 hover:bg-green-100">
                      Copy comment
                    </button>
                    <button onClick={() => alert('Open Facebook/Instagram → find post → 3 dots → Report → False information')}
                      className="text-xs px-3 py-1 rounded border border-red-200 bg-red-50 text-red-700 hover:bg-red-100">
                      Report to Meta
                    </button>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800 leading-relaxed">
              <strong>Admin policy:</strong> The AI flags posts automatically. Humans review and decide action.
              The app never auto-deletes or auto-bans. You can copy the comment and post it manually,
              or report to Meta via their official report flow. For groups you admin, you can delete directly from Facebook.
            </div>
          </div>
        )}
      </main>
