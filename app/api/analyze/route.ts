  ...
  try {
    ...
    const langInstruction =
      lang === 'uz'
        ...
        : lang === 'tj' ? `CRITICAL LANGUAGE INSTRUCTION: You MUST write ALL of the following fields ENTIRELY in Tajik language using Tajik Cyrillic script: claim_summary, analysis, authentic_alternative, red_flags (every item), references (description field only), and suggested_comment. Do NOT use Uzbek, Russian, or English anywhere in these fields. Every single sentence — including the last sentence — must be in Tajik. When referring to the Prophet, always write (с.а.в) in Tajik Cyrillic. Only keep JSON field names, source names, URLs, and verdict/confidence/severity values in English.`
